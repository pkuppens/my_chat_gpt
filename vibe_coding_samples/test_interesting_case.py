...
def test_interesting_case():
    ...
    # Create solver
    solver = KnightBishopSolver(
        TEST_CASE['board_size'][0],
        TEST_CASE['board_size'][1],
        Position(*TEST_CASE['bishop_pos'])
    )
    ...
    for method in ['bfs', 'bidirectional_bfs', 'a_star', 'ida_star']:
        ...
        if method == 'bfs':  # Only BFS should succeed
            assert result == TEST_CASE['expected_moves'], \
                f"BFS should find {TEST_CASE['expected_moves']} moves, but found {result}"
